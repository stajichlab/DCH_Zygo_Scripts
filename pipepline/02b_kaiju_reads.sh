...
#SBATCH --output=kaiju_out_reads.log
#SBATCH -p highmem

module load kaiju

for i in Rmicrosporus_OMAR/raw_reads/*.gz
do
  	b=$(basename "$i" .fa)                                                                                                                                           
        echo "$b"                                                                                                                                                        
        kaiju -v -t kaijudb/nodes.dmp -f kaijudb/nr/kaiju_db_nr.fmi -z 25 -a "greedy" -i $i -o kaiju_out/$b.ko
done
